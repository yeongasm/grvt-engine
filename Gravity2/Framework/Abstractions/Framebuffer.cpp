...
bool FramebufferCreationInfo::AddAttachment(AttachComponent Component, AttachmentType Type)
{
	...
	for (Pair<AttachComponent, AttachmentType>& Attachment : Attachments)
	{
...
GrvtFramebuffer::Attachment::Attachment(GrvtFramebuffer::Attachment&& Rhs) { *this = Gfl::Move(Rhs); }
...
void GrvtFramebuffer::Alloc(const FramebufferCreationInfo& Info) 
{
	...
	for (Pair<AttachComponent, AttachmentType>& Attach : Info.Attachments)
	{
